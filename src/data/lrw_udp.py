...
class MultiDataset(Dataset):
    def __init__(self, lrw, mode, max_v_timesteps=155, augmentations=False, num_mel_bins=80, subject=0, adapt_min=1, fold=1):
        assert mode in ['train', 'test', 'adapt', 'val']
        assert adapt_min in [1, 3, 5]
        self.mode = mode
        self.fold = fold
        self.max_v_timesteps = max_v_timesteps
        self.augmentations = augmentations if (mode != 'test' and mode != 'val') else False
    ...
    def build_file_list(self, lrw, mode, target_subject, adapt_min):
        ...
        sub_num = -1
        data_name = 'val' if mode == 'adapt' else mode
        with open(f'./data/LRW_ID_{data_name}.txt', 'r') as f:
            lines = f.readlines()
        for l in lines:
        ...
        if mode == 'adapt':
            ...
            print(f'subject ind:{target_subject} #{check[target_subject]}, {mode} with fold {self.fold}, Use data of {adapt_min} min, so {len(file_list)} data')
        elif mode == 'val':
            file_list = file_list[round(adapt_min * 60 / 1.16):]
            print(f'subject ind:{target_subject} #{check[target_subject]}, {mode} with fold {self.fold}, Validate on remaining adaptation set, so {len(file_list)} data')
        else:
