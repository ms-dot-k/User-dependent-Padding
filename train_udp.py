...
from src.data.multiepoch_loader import MultiEpochsDataLoader
...
def parse_args():
    ...
    parser.add_argument("--total_step", type=int, default=400)  #200: 1min, 300: 3min, 400: 5min
...
def train_net(args):
    ...
    args.checkpoint_dir = args.checkpoint_dir + f'/s{args.subject}/{args.adapt_min}min_lr{args.lr}_fold{args.fold}'
    ...
    while step < args.total_step:
        ...
        for i, batch in enumerate(dataloader):
            ...
            if step % args.eval_step == 0:
                if args.local_rank == 0:
                    logs = validate(v_front, tcn, udps, step=step, writer=writer, dataloader=val_dataloader)
                else:
                    logs = validate(v_front, tcn, udps, step=step, writer=None, dataloader=val_dataloader)
...
def validate(v_front, tcn, udps, fast_validate=False, step=0, writer=None, dataloader=None):
    with torch.no_grad():
        v_front.eval()
        tcn.eval()
